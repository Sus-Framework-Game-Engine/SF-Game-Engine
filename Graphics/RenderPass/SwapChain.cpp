...
namespace SF::Engine
{
                                                                                                                                                                                      ...
                                                                                                                                                                                      activeImageIndex(std::numeric_limits<uint32_t>::max())
    {
        ...
        {
            std::array<uint32_t, 2> queueFamily = {RenderSystemFamily, presentFamily};
        ...
        RenderSystem::CheckVkResult(vkCreateSwapchainKHR(logicalDevice, &swapchainCreateInfo, nullptr, &swapchain));

        RenderSystem::CheckVkResult(vkGetSwapchainImagesKHR(logicalDevice, swapchain, &imageCount, nullptr));
        images.resize(imageCount);
        imageViews.resize(imageCount);
        RenderSystem::CheckVkResult(vkGetSwapchainImagesKHR(logicalDevice, swapchain, &imageCount, images.data()));
    ...
    VkResult Swapchain::AcquireNextImage(const VkSemaphore &presentCompleteSemaphore, VkFence fence)
    {
        ...
        // RenderSystem::CheckVkResult(vkWaitForFences(*logicalDevice, 1, &fenceImage, VK_TRUE, std::numeric_limits<uint64_t>::max()));

        // RenderSystem::CheckVkResult(vkResetFences(*logicalDevice, 1, &fenceImage));
