...
namespace SF::Engine
{
    ...
    Image::Image(VkFilter filter,
                 ...
                 const VkExtent3D &extent) : extent(extent),
                                             samples(samples),
                                             usage(usage),
                                             format(format),
                                             mipLevels(mipLevels),
    ...
    WriteDescriptorSetInformation Image::GetWriteDescriptor(uint32_t binding, VkDescriptorType descriptorType, const std::optional<OffsetSize> &offsetSize) const
    {
    ...
    void Image::CreateImage(VkImage &image, VmaAllocation &allocation, const VkExtent3D &extent, VkFormat format, VkSampleCountFlagBits samples,
                            VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, uint32_t mipLevels, uint32_t arrayLayers, VkImageType type)
    {
        ...
        allocCreateInfo.requiredFlags = properties; // Use properties instead of usage

        VmaAllocationInfo allocInfo = {}; // Declare this
    ...
    void Image::CreateImageSampler(VkSampler &sampler, VkFilter filter, VkSamplerAddressMode addressMode, bool anisotropic, uint32_t mipLevels)
    {
        ...
        RenderSystem::CheckVkResult(vkCreateSampler(*logicalDevice, &samplerCreateInfo, nullptr, &sampler));
    }
                                ...
                                uint32_t mipLevels, uint32_t baseMipLevel, uint32_t layerCount, uint32_t baseArrayLayer)
    {
        ...
        RenderSystem::CheckVkResult(vkCreateImageView(*logicalDevice, &imageViewCreateInfo, nullptr, &imageView));
    }
    ...
    bool Image::CopyImage(const VkImage &srcImage, VkImage &dstImage, VmaAllocation &alloc, VkFormat srcFormat, const VkExtent3D &extent,
                          VkImageLayout srcImageLayout, uint32_t mipLevel, uint32_t arrayLayer)
    {
        ...
        CreateImage(dstImage, alloc, extent, VK_FORMAT_R8G8B8A8_UNORM, VK_SAMPLE_COUNT_1_BIT, VK_IMAGE_TILING_LINEAR,
