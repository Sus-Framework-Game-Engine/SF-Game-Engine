...
namespace SF::Engine
{
     ...
     */
    class RenderSystemPipeline : public Pipeline
    {
    public:
        enum class Mode
        {
         ...
         */
        RenderSystemPipeline(Stage stage,
        ...
        const std::filesystem::path &GetShaderPath() const { return shaderPath; }
    ...
    class RenderSystemPipelineCreate
    {
    public:
        RenderSystemPipelineCreate(std::filesystem::path shaderPath = {},
                                   std::vector<Shader::VertexInput> vertexInputs = {},
                                   std::vector<Shader::Define> defines = {},
                                   RenderSystemPipeline::Mode mode = RenderSystemPipeline::Mode::Polygon,
                                   RenderSystemPipeline::Depth depth = RenderSystemPipeline::Depth::ReadWrite,
                                   VkPrimitiveTopology topology = VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST,
                                   VkPolygonMode polygonMode = VK_POLYGON_MODE_FILL,
                                   VkCullModeFlags cullMode = VK_CULL_MODE_BACK_BIT,
                                   VkFrontFace frontFace = VK_FRONT_FACE_CLOCKWISE,
         ...
         */
        RenderSystemPipeline *Create(const Pipeline::Stage &pipelineStage) const
        {
            return new RenderSystemPipeline(pipelineStage, shaderPath, vertexInputs, defines, mode, depth,
        ...
        const std::vector<Shader::VertexInput> &GetVertexInputs() const { return vertexInputs; }
        const std::vector<Shader::Define> &GetDefines() const { return defines; }
        RenderSystemPipeline::Mode GetMode() const { return mode; }
        RenderSystemPipeline::Depth GetDepth() const { return depth; }
        VkPrimitiveTopology GetTopology() const { return topology; }
        VkPolygonMode GetPolygonMode() const { return polygonMode; }
        VkCullModeFlags GetCullMode() const { return cullMode; }
        VkFrontFace GetFrontFace() const { return frontFace; }
    ...
    private:
        ...
        RenderSystemPipeline::Mode mode;
        RenderSystemPipeline::Depth depth;
        VkPrimitiveTopology topology;
        VkPolygonMode polygonMode;
        VkCullModeFlags cullMode;
        VkFrontFace frontFace;
