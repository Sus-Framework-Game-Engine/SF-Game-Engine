...
namespace rocket
{
    ...
    namespace detail
    {
        ...
        struct connection_scope final
        {
            connection_scope(void *base, bool is_thread_safe, thread_local_data *th) noexcept
                : th{th}, prev{th->current_connection}, prevThreadSafe{th->is_thread_safe_connection}
            {
    ...
    {
        connection() noexcept
            : base{nullptr}, is_thread_safe{false}
        {
        }
    ...
    namespace detail
    {
        ...
        struct timer_queue final
        {
            ...
            {
                connection c{set_interval<Rep, Period>([&object, method]
            ...
            {
                connection c{set_interval<Rep, Period>([&object]
                                                       { (object.*Method)(); }, interval)};
                if constexpr (std::is_convertible_v<Instance *, trackable *>)
                {
                    static_cast<trackable &>(object).add_tracked_connection(c);
            ...
            {
                connection c{set_timeout<Rep, Period>([&object, method]
            ...
            {
                connection c{set_timeout<Rep, Period>([&object]
                                                      { (object.*Method)(); }, timeout)};
                if constexpr (std::is_convertible_v<Instance *, trackable *>)
                {
                    static_cast<trackable &>(object).add_tracked_connection(c);
    ...
    struct signal<R(Args...), Collector, ThreadingPolicy> final
    {
        ...
        connection connect(R1 (*method)(Args1...), connection_flags flags = direct_connection)
        {
            return connect([method](Args const &...args)
                           { return R((*method)(Args1(args)...)); }, flags);
        }
        ...
        connection connect(Instance &object, R1 (Class::*method)(Args1...), connection_flags flags = direct_connection)
        {
            connection c{connect(
                ...
                { return R((object.*method)(Args1(args)...)); }, flags)};
            if constexpr (std::is_convertible_v<Instance *, trackable *>)
            {
                static_cast<trackable &>(object).add_tracked_connection(c);
            }
        ...
        {
            connection c{connect([&object](Args const &...args)
                                 { return R((object.*Method)(args...)); }, flags)};
            if constexpr (std::is_convertible_v<Instance *, trackable *>)
            {
                static_cast<trackable &>(object).add_tracked_connection(c);
            }
                ...
                while (current != end)
                {
                    ...
                    if (current->prev != nullptr
                        ...
                        ROCKET_LIKELY
                        {
                            ...
                            functional_connection *conn = std::launder(static_cast<functional_connection *>(static_cast<void *>(current)));
                                                ...
                                                [current, args = detail::make_tuple(args...)]
                                                {
                                                    if (current->is_connected())
                                                        ROCKET_LIKELY
                                                        {
                                                            ...
                                                            functional_connection *conn = std::launder(static_cast<functional_connection *>(
