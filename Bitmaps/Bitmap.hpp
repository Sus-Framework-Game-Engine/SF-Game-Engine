...
namespace SF::Engine
{
    ...
    public:
        ...
        Bitmap(std::unique_ptr<uint8_t[]> &&data, const Vector2Uint &size, uint32_t bytesPerPixel = 4);
        ~Bitmap() = default;
        ...
        const Vector2Uint &GetSize() const { return size; }
        void SetSize(const Vector2Uint &size) { this->size = size; }
